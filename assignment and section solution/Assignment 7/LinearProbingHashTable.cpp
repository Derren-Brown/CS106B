...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    while (i < capacity)
    {
        if (elems[(idx + i) % capacity].type != SlotType::FILLED)
        {
            elems[(idx + i) % capacity].type = SlotType::FILLED;
            elems[(idx + i) % capacity].value = elem;
            num++;
            return true;
        }
        i++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    int idx = hash(elem), i = 0;
    while (elems[(idx + i) % capacity].type != SlotType::EMPTY && i < capacity)
    {
        if (elems[(idx + i) % capacity].value == elem && elems[(idx + i) % capacity].type == SlotType::FILLED)
        {
            return true;
        }
        i++;
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    while (elems[(idx + i) % capacity].type != SlotType::EMPTY && i < capacity)
    {
        if (elems[(idx + i) % capacity].value == elem && elems[(idx + i) % capacity].type == SlotType::FILLED)
        {
            elems[(idx + i) % capacity].type = SlotType::TOMBSTONE;
            num--;
            return true;
        }
        i++;
