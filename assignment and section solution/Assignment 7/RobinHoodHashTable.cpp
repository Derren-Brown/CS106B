...
bool RobinHoodHashTable::insert(const string& elem) {
    if (contains(elem) || num == capacity)
    {
        return false;
    }
    int idx = hash(elem), i = 0;
    string myElem = elem;
    while (i < capacity) // idx denotes the current location and i denotes distance
    {
        // Base case: the slot is empty
        if (elems[idx % capacity].distance == EMPTY_SLOT)
        {
            elems[idx % capacity] = {myElem, i};
            num++;
            return true;
        }
        /* Recursive case: the slot isn't empty but distance of the elem in the slot is lower
         * Now we swap the distance and value and continue the loop
         */
        else if (i > elems[idx % capacity].distance)
        {
            swap(elems[idx % capacity].distance, i);
            swap(elems[idx % capacity].value, myElem);
        }
        idx++;
        i++;
    }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    while (i < capacity)
    {
        if (i > elems[(idx + i) % capacity].distance)
        {
            break;
        }
        if (elems[(idx + i) % capacity].distance != EMPTY_SLOT && elems[(idx + i) % capacity].value == elem)
        {
            return true;
        }
        i++;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while (true)
    {
        if (elems[idx % capacity].distance != EMPTY_SLOT && elems[idx % capacity].value == elem)
        {
            elems[idx % capacity].distance = EMPTY_SLOT;
            while (elems[(idx + 1) % capacity].distance && elems[(idx + 1) % capacity].distance != EMPTY_SLOT)
            {
                elems[idx % capacity].distance = elems[(idx + 1) % capacity].distance - 1;
                elems[idx % capacity].value = elems[(idx + 1) % capacity].value;
                idx++;
            }
            elems[idx % capacity].distance = EMPTY_SLOT; // Zero the last element
            num--;
            return true;
        }
        idx++;
