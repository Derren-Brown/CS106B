...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    for (int i = 0; i + kGramLength - 1< length; i++)
    {
        string substr = str.substr(i, kGramLength);
        if (kGrams.containsKey(substr))
        {
            kGrams[substr]++;
        }
        else
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string key: output)
    {
        if (output[key])
        {
            square_sum += pow(output[key], 2);
            flg = 1;
        }
    }
    if (!flg)
    {
        error("The map doesn't contain non-zero values.");
    }
    for (string key: output)
    {
        output[key] /= sqrt(square_sum);
    }
    return output;
}

Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    PriorityQueue<string> grams;
    ...
    else if (source.size() <= numToKeep)
    {
        return source;
    }
    for (string key: source)
    {
        grams.enqueue(key, source[key]);
        if (grams.size() > numToKeep)
        {
            grams.dequeue();
        }
    }
    while (!grams.isEmpty())
    {
        string str = grams.dequeue();
        output.put(str, source[str]);
    }
    return output;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    string lang = {};
    Map <string, double> langSimilarity = {};
    double max = 0;
    for (struct Corpus elem: corpora)
    {
        double similarity = cosineSimilarityOf(textProfile, elem.profile);
        if (max < similarity)
        {
            max = similarity;
            lang = elem.name;
