...
Queue<GridLocation> createQueue(const Grid<double>& terrain,
                                const Vector<GridLocation>& sources,
                                double height);
GridLocation createGridLocation(int row, int col);
void flood(Grid<bool>& floodedRegions, const Grid<double>& terrain,
           Queue<GridLocation>& queue, int row, int col, double height);

Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            const Vector<GridLocation>& sources,
                            double height) {
    Queue<GridLocation> queue = createQueue(terrain, sources, height);
    int nrows = terrain.numRows(), ncols = terrain.numCols();
    Grid<bool> floodedRegions(nrows, ncols, false);
    while (!queue.isEmpty())
    {
        ...
        if (row > 0)
        {
            flood(floodedRegions, terrain, queue, row - 1, col, height);
        }
        if (row < nrows - 1)
        {
            flood(floodedRegions, terrain, queue, row + 1, col, height);
        ...
        if (col > 0)
        {
            flood(floodedRegions, terrain, queue, row, col - 1, height);
        }
        if (col < ncols - 1)
        {
           flood(floodedRegions, terrain, queue, row, col + 1, height);
...
void flood(Grid<bool>& floodedRegions, const Grid<double>& terrain,
           Queue<GridLocation>& queue, int row, int col, double height)
{
    if (floodedRegions[row][col] == false && terrain[row][col] <= height)
    {
        queue.enqueue(createGridLocation(row, col));
