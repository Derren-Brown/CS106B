...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork,
                                  const int numCities, int currentNum,
                                  Set<string>& supplyLocations, Set<string>& left);

bool isValid(const Map<string, Set<string>>& roadNetwork,
             const int numCities,int currentNum,
             Set<string>& supplyLocations, string& city, Set<string>& left);
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    Set<string> left;
    for (string key: roadNetwork)
    {
        left.add(key);
    }
    return canBeMadeDisasterReadyHelper(roadNetwork, numCities, 0, supplyLocations, left);
}
...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork,
                                  ...
                                  Set<string>& supplyLocations, Set<string>& left)
{
    /* "currentNum" records the current number of supply cities.
     * "left" records the current uncovered cities.
     * */
    // Recursive case
    for (string elem: roadNetwork)
    {
        if (!isCovered(elem, roadNetwork, supplyLocations))
        {
            string city = left.first(); // Each time we iterate one uncovered city
            if (isValid(roadNetwork, numCities, currentNum + 1, supplyLocations, city, left)) // case: city itself
            {
                return true;
            }
            for (string neighbor: roadNetwork[city])
            {
                if (isValid(roadNetwork, numCities, currentNum + 1, supplyLocations, neighbor, left)) // case: city's neighbor
                {
                    return true;
                }
            }
...
bool isValid(const Map<string, Set<string>>& roadNetwork,
             ...
             Set<string>& supplyLocations, string& city, Set<string>& left)
{
    // Abstract the same steps to a city and its neighbor (in function "canBeMadeDisasterReadyHelper")
    if (currentNum > numCities) // Consider the num limit
    {
        return false;
    }
    Set<string> leftCopy = left;
    // NOTE that our following operations to left and supplyLocations are different
    left.remove(city);
    supplyLocations += city;
    for (string elem: roadNetwork[city])
    {
        left.remove(elem);
    }
    if (canBeMadeDisasterReadyHelper(roadNetwork, numCities, currentNum, supplyLocations, left))
    {
        return true;
    }
    supplyLocations -= city;
...
STUDENT_TEST("Works for map with one city at the centre and the other three connected to it")
{
