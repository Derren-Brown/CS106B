...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    Set<string> people;
    for (string elem: possibleLinks)
    {
        people.add(elem);
    }
    return hasPerfectMatchingHelper(possibleLinks, matching, people);
}
...
{
    // Base case
    if (matching.size() * 2 == possibleLinks.size())
    {
        return true;
    }
    // Recursive case
    Set<string> leftCopy = left;
    for (string person1: leftCopy)
    {
        for (string person2: possibleLinks[person1])
        {
            if (left.contains(person2))
            {
                Pair greek(person1, person2);
                matching.add(greek);
                left.remove(person1);
                left.remove(person2);
                if (hasPerfectMatchingHelper(possibleLinks, matching, left))
                {
                    return true;
                }
                matching.remove(greek);
                left.add(person1);
                left.add(person2);
            }
        }
...
void maximumWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks,
                                 Set<Pair>& pairs, Set<string>& left, int weight, int& maxWeight,
                                 Set<Pair>& maxWeightPair, Set<Pair> record);
void setMaxWeightPair(int weight, Set<Pair>& pairs, int& maxWeight, Set<Pair>& maxWeightPair);

Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    Set<Pair> pairs, maxWeightPair;
    Set<string> left;
    ...
    int maxWeight = 0;
    maximumWeightMatchingHelper(possibleLinks, pairs, left, 0, maxWeight, maxWeightPair, {});
...
{
    /*
     * NOTE that the "record" is passed by value and that is extremely important in the case:
     *     It means that the memorization only works in the original function
     *     and we only record the pair in the original function.
     * If you pass the record by reference in this case, you will pass the test BUT you're actually wrong.
     */
    Set<string> leftCopy = left;
    for (string person1: leftCopy)
    {
        for (string person2: possibleLinks[person1])
        {
            if (left.contains(person2))
            {
                ...
                pairs.add(greek);
                // Memorization
                if (record.contains(greek))
                {
                    setMaxWeightPair(weight + possibleLinks[person1][person2], pairs, maxWeight, maxWeightPair);
                    pairs.remove(greek);
                    continue;
                }
                record.add(greek);
                left.remove(person1);
                left.remove(person2);
                weight += possibleLinks[person1][person2];
                setMaxWeightPair(weight, pairs, maxWeight, maxWeightPair);
                maximumWeightMatchingHelper(possibleLinks, pairs, left, weight, maxWeight, maxWeightPair, record);
                weight -= possibleLinks[person1][person2];
                left.add(person1);
                left.add(person2);
                pairs.remove(greek);
...
STUDENT_TEST("hasPerfectMatching works on a world with 4 people.")
{
    /* The world is consisted of 4 people.
     *
     *               A --- B
     *               |   / |
     *               |  /  |
     *               C --- D
     *
     * The matching case would be AB/CD, AC/BD.
     */
