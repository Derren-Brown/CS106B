 ...
 */
void createMap(Map<char, int>& map, const string& str);
void createQueue(PriorityQueue<EncodingTreeNode*>& queue, Map<char, int>& map);
EncodingTreeNode* combine(EncodingTreeNode*& elem1, EncodingTreeNode*& elem2);
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if (map.size() < 2)
    {
        error("Too few distinct elements.");
    }
    createQueue(queue, map);
    while (queue.size() > 1)
    {
        EncodingTreeNode *elem1, *elem2, *elem3;
        int priority = queue.peekPriority();
        elem1 = queue.dequeue();
        priority += queue.peekPriority();
        elem2 = queue.dequeue();
        elem3 = combine(elem1, elem2);
        queue.enqueue(elem3, priority);
    }
...
void createQueue(PriorityQueue<EncodingTreeNode*>& queue, Map<char, int>& map)
{
    for (char key: map)
    {
        ...
        node->one = nullptr;
        node->zero = nullptr;
        queue.enqueue(node, map[key]);
    }
}

EncodingTreeNode* combine(EncodingTreeNode*& elem1, EncodingTreeNode*& elem2)
{
    EncodingTreeNode* elem3 = new EncodingTreeNode;
    elem3->zero = elem1;
    elem3->one = elem2;
    return elem3;
}
 ...
 */
void createRecord(Map<string, char>& record, EncodingTreeNode*& tree, string s);
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    Map<string, char> fromCodeTochar;
    createRecord(fromCodeTochar, tree, "");
    string s = "", text = "";
    while (!bits.isEmpty())
    {
        if (bits.dequeue() == 1)
        ...
        if (fromCodeTochar.containsKey(s))
        {
            text += fromCodeTochar[s];
            s = "";
        }
    }
    return text;
...
void createRecord(Map<string, char>& record, EncodingTreeNode*& tree, string s)
{
    ...
    {
        record.put(s, tree->ch);
        return;
    }
    if (tree->zero != nullptr)
    {
        createRecord(record, tree->zero, s + "0");
    }
    if (tree->one != nullptr)
    {
        createRecord(record, tree->one, s + "1");
    }
}
/**
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    Map<string, char> fromCodeTochar;
    Map<char, string> fromCharToCode;
    createRecord(fromCodeTochar, tree, "");
    for (string key: fromCodeTochar)
    {
        fromCharToCode.put(fromCodeTochar[key], key);
    }
    Queue<Bit> bits;
    for (char ch: str)
    {
        for (char c: fromCharToCode[ch])
        {
            if (c == '0')
            {
                bits.enqueue(0);
...
HuffmanResult compress(const string& text) {
    ...
    Queue<Bit> messageBits = encodeText(text, tree), treeBits;
...
string decompress(HuffmanResult& file) {
    ...
    deleteTree(tree);
    return decodeText(messageBits, tree);
}
