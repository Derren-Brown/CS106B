 ...
 */
Set<string> recursion(Vector<string> sentence, int point);
string vectorToString(Vector<string>& sentence);

Set<string> allEmphasesOf(const string& sentence) {
    return recursion(tokenize(sentence), 0);
}

Set<string> recursion(Vector<string> sentence, int point)
{
    Set<string> sentences;
    if (point == sentence.size())
    {
        sentences.add(vectorToString(sentence));
    }
    else if (!isalpha(sentence[point][0]))
    {
        sentences += recursion(sentence, point + 1);
    }
    else
    {
        sentence[point] = toLowerCase(sentence[point]);
        sentences += recursion(sentence, point + 1);
        sentence[point] = toUpperCase(sentence[point]);
        sentences += recursion(sentence, point + 1);
    }
    return sentences;
}
...
STUDENT_TEST("A simple test.")
{
    ...
    EXPECT_EQUAL(allEmphasesOf("good morning."), expected);
}
