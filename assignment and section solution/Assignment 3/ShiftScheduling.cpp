 ...
 */
void recursion(const Vector<Shift>& enumeration, Set<Shift> schedule, int maxHours, int point,
                          Set<Shift>& highestValueSchedule, int& maxValue);
bool isValid(const Set<Shift>& ans, const Shift& prepare, int maxHours);
int computeValue(const Set<Shift>& schedule);
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0)
    {
        error("Maxhours is negative.");
    }
    Vector<Shift> enumeration;
    for (Shift elem: shifts)
    {
        enumeration.add(elem);
    }
    Set<Shift> schedule;
    Set<Shift> highestValueSchedule;
    int maxValue = 0;
    recursion(enumeration, schedule, maxHours, 0, highestValueSchedule, maxValue);
...
void recursion(const Vector<Shift>& enumeration, Set<Shift> schedule, int maxHours, int point,
                          Set<Shift>& highestValueSchedule, int& maxValue)
{
    if (point == enumeration.size())
    {
        int val = computeValue(schedule);
        if (maxValue < val)
        {
            maxValue = val;
    ...
    {
        recursion(enumeration, schedule, maxHours, point + 1, highestValueSchedule, maxValue);
        if (isValid(schedule, enumeration[point], maxHours))
        {
            schedule.add(enumeration[point]);
            recursion(enumeration, schedule, maxHours, point + 1, highestValueSchedule, maxValue);
        }
    }
    return;
...
bool isValid(const Set<Shift>& ans, const Shift& prepare, int maxHours)
{
    int hours = lengthOf(prepare);
    if (ans.isEmpty())
    {
        if (lengthOf(prepare) > maxHours)
        {
            return false;
        }
    }
    ...
    {
        hours += lengthOf(elem);
        if (overlapsWith(elem, prepare) || hours > maxHours)
        {
            return false;
        }
    }
...
STUDENT_TEST("Don't pick only shift if you don't have time for it.")
{
    Set<Shift> shifts = {
        { Day::MONDAY, 9, 17, 1000 },  // Monday, 9AM - 5PM, value is 1000
    };

    EXPECT_EQUAL(highestValueScheduleFor(shifts, 3), {});
}
