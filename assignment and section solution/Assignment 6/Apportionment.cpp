...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    {
        error("numSeats is less than the number of states.");
    }
    class HeapPQueue queue;
    Map<string, int> recd;
    for (string key: populations)
    {
        queue.enqueue({key, -populations[key] / sqrt(2)});
        recd.put(key, 1); // Initially give each state a seat
        numSeats--;
    }
    while (numSeats)
    {
        DataPoint state = queue.dequeue();
        recd[state.name]++;
        numSeats--;
        int s = recd[state.name];
        state.weight = -populations[state.name] / sqrt(s * (s + 1));
        queue.enqueue(state);
    }
    return recd;
}
