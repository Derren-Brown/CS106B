...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    {
        allocatedSize *= 2;
        DataPoint* pointer = new DataPoint[allocatedSize];
    ...
    int n = logicalSize;
    while (n > 1 && elems[n].weight < elems[n / 2].weight)
    {
...
DataPoint HeapPQueue::dequeue() {
    ...
    DataPoint elem = elems[1];
    swap(elems[1], elems[logicalSize]);
    logicalSize--;
    int n = 1;
    while (2 * n + 1 <= logicalSize)
    {
        if (elems[2 * n].weight < elems[2 * n + 1].weight)
        {
            if (elems[n].weight > elems[2 * n].weight)
            {
                swap(elems[n], elems[2 * n]);
                n *= 2;
            }
            else break;
        }
        ...
        {
            if (elems[n].weight > elems[2 * n + 1].weight)
            {
                swap(elems[n], elems[2 * n + 1]);
                n = 2 * n + 1;
            }
            else break;
        }
