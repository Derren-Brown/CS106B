 ...
 * a lot fewer!
 */
int fewestCoinsForHelper(int cents, const Vector<int>& coins, int idx, int curnum);
int fewestCoinsFor(int cents, const Set<int>& coins) {
    Vector<int> coinsAfter;
    for (int coin: coins)
    {
        coinsAfter.add(coin);
    }
    return fewestCoinsForHelper(cents, coinsAfter, 0, 0);
}
...
int fewestCoinsForHelper(int cents, const Vector<int>& coins, int idx, int curnum)
{
    ...
    int coin = coins[idx], i = 0, minnum = 100;
    while (cents - i * coin > -1)
    {
        minnum = min(minnum, fewestCoinsForHelper(cents - i * coin, coins, idx + 1, curnum + i));
        i++;
    }
    return minnum;
}
