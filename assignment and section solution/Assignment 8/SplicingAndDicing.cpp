...
Nucleotide* toStrand(const string& str) {
    ...
    dna->next = nullptr;
    Nucleotide* cur = dna;
    for (char key: str.substr(1))
    {
        Nucleotide* next = new Nucleotide;
        cur->next = next;
        next->prev = cur;
        next->value = key;
        next->next = nullptr;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    Nucleotide *idx1 = dna, *idx2 = target;
    while (dna != nullptr)
    {
        idx1 = dna;
        while (idx1 != nullptr && idx2 != nullptr && idx1->value == idx2->value)
        {
            idx1 = idx1->next;
            idx2 = idx2->next;
        }
 ...
 * of nucleotides given as the start parameter. If the nucleotide pointed at
 * by start was removed, start is updated to point to the first nucleotide after
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide *idx1 = dna, *start = dna, *idx2 = target;\
    // search
    while (start != nullptr)
    {
        idx1 = start;
    ...
    if (idx2 == nullptr)
    {
        if (idx1 == nullptr)
        {
            ...
            {
                Nucleotide* prev = start->prev;
                prev->next = nullptr;
            }
        }
        else
        {
            Nucleotide *end = idx1->prev;
            end->next = nullptr;
            if (start == dna)
            {
                idx1->prev = nullptr;
...
STUDENT_TEST("fromDNA handles multicharacter strings.")
{
