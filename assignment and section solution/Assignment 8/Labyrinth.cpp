#include "Labyrinth.h"
using namespace std;
bool isValid(MazeCell*& start, bool& spellbook, bool& potion, bool& wand);
bool isPathToFreedom(MazeCell* start, const string& moves) {
    bool spellbook = false, potion = false, wand = false;
    for (char key: moves)
    {
        if (!isValid(start, spellbook, potion, wand))
        {
            return false;
        }
        switch (key)
        {
        case 'N': start = start->north;break;
        case 'S': start = start->south;break;
        case 'E': start = start->east;break;
        case 'W': start = start->west;break;
        }
    }
    if (!isValid(start, spellbook, potion, wand)) // Handle the last case
    {
        return false;
    }
