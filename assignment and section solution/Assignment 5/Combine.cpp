...
Vector<DataPoint> merge(const Vector<DataPoint>& sequence1, const Vector<DataPoint>& sequence2);
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    Vector<DataPoint> sequence1, sequence2;
    ...
    {
        sequence1 = combine(sequences.subList(0, size / 2));
        sequence2 = combine(sequences.subList(size / 2));
    }
...
Vector<DataPoint> merge(const Vector<DataPoint>& sequence1, const Vector<DataPoint>& sequence2)
{
    ...
    int idx1 = 0, idx2 = 0;
    while (idx1 < sequence1.size() && idx2 < sequence2.size())
    {
        if (sequence1[idx1].weight < sequence2[idx2].weight)
        {
            result += sequence1[idx1];
            idx1++;
        }
        else
        {
            result += sequence2[idx2];
            idx2++;
        }
    }
    if (idx1 < sequence1.size())
    {
        result += sequence1.subList(idx1);
    }
    else if (idx2 < sequence2.size())
    {
        result += sequence2.subList(idx2);
    }
    return result;
}
...
STUDENT_TEST("Merges two two-item sequences.")
{
